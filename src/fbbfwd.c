   ...
   if (Mbheader)
     {
        ...
        fprintf (tfile, " #:%ld", (f->ind.msgid % 100000L));  /* n5knx: use rightmost 5 digits */
        /* The BID, if any */
        if (f->bid[0] != '\0')
            ...
            int    idx)
{
  ...
  j2alarm(Mbtdiscinit*1000);
   ...
   /* log (-1, "about to validate FS line"); */
...
/* 18Apr2008, Maiko, B2F has specific headers that need to be put in */
	...
	/* 07Sep2013, Maiko, Need to deal with 'R' for reject (B1F) */
         if ((fsr == '-') || (fsr == 'N') || (fsr == 'R'))
		{
            ...
            logmbox (m->user, m->name, "refused %s", msglst[i].sline);
         }
		else
   ...
   if(recvline(m->user,m->line,MBXLINE) == -1) {
      ...
      for(i=0;i<FBBMAXMSGS;i++) {
         if(msglst[i].accept == fbbYES) {
            ...
            // For each valid note.
            logmbox (m->user, m->name, "sent %s", msglst[i].sline);
...
#ifdef	DONT_COMPILE
/*
 * 21Sep2006, Maiko, For some reason I'm getting checksum errors when
 * trying to forward using the FA bid. Comment this out, and it works
 * pretty good actually ! I think I actually have this figured out now
 * as far as flow control goes. Keep this DONT_COMPILE for now.
 */
		...
		if (!strnicmp (m->line, "fa", 2) || !strnicmp (m->line, "fc", 2))
		{
           ...
           if(msgcnt >= FBBMAXMSGS)
              ...
              while (*cp)
			{
              ...
              checksum += '\r' & 0xff;
#ifdef FBBVERBOSELOG
              logmbox (m->user, m->name, "%s", m->line);
#endif
...
static int doB2Facceptnote (struct mbx *m, char *infile)
{
	...
	while (fgets (m->line, MBXLINE, fpInFile))
	{
		...
		if (!strncmp (m->line, "To: ", 4))
		{
			...
			{
				log (-1, "main recipient [%s] CC'd instead", m->line + 4);
	...
	if (validate_address (m->to) == 0)
		log (-1, "validate_address [%s] failed", m->to);
	...
	while (bodysize > 0)
	{
		...
		{
			log (-1, "body ended earlier than I expected");
