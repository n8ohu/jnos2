 ...
 */
static int docallslist (int argc, char *argv[], void *p);
 ...
 */
static int callsign_in_list (char **tptr, char *callsign)
{
	char *lptr, *cptr;
	int matched = 0;
...
static void usagecallslist (void)
{
	tprintf ("Usage: aprs calls [fwdtorf|bantorf|ignorerf|postorf|stattorf|wxtorf|micetorf|obj2rf] *[callsign &| pattern list]\n");
	tprintf ("       aprs calls ip45845 *[list of ip addresses allowed msg'ng]\n");
}

static int docallslist (int argc, char *argv[], void *p OPTIONAL)
{
	char **tptr, ***topptr;
	...
	if ((*topptr = (char**)malloc (sizeof(char*) * (argc+3))) == (char**)0)
	{
	...
	for (tptr = *topptr, cnt = 0; cnt < (argc - 2); cnt++)
	{
		*tptr = j2strdup (argv[cnt+2]);
