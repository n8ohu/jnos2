...
static void lcp_option __ARGS((struct mbuf **bpp,struct lcp_value_s *value_p,
    byte_t o_type,byte_t o_length,struct mbuf **copy_bpp ));
static void lcp_makeoptions __ARGS((struct mbuf **bpp,struct lcp_value_s *value_p,
    int16 negotiating));
static struct mbuf *lcp_makereq __ARGS((struct fsm_s *fsm_p));
  
static int lcp_check __ARGS((struct mbuf **bpp,struct lcp_s *lcp_p,
    struct lcp_side_s *side_p,struct option_hdr *option_p,int request));
  
static int lcp_request  __ARGS((struct fsm_s *fsm_p,struct config_hdr *config,
    struct mbuf *data));
static int lcp_ack  __ARGS((struct fsm_s *fsm_p,struct config_hdr *ackcnf,
    struct mbuf *data));
static int lcp_nak  __ARGS((struct fsm_s *fsm_p,struct config_hdr *nakcnf,
    struct mbuf *data));
static int lcp_reject   __ARGS((struct fsm_s *fsm_p,struct config_hdr *rejcnf,
    struct mbuf *data));
...
static struct cmds DFAR Lcpcmds[] = {
...
static struct cmds DFAR Lcpside_cmds[] = {
...
struct fsm_s *fsm_p;
{
    ...
    if (lcp_p->local.want.negotiate & LCP_N_AUTHENT) {  /* did we require authorization? */
        switch (lcp_p->local.want.authentication) {
            case PPP_PAP_PROTOCOL:
                pap_local(fsm_p->ppp_p);
                break;
        };
    }
    if (lcp_p->remote.want.negotiate & LCP_N_AUTHENT) { /* ?? symmetry */
        switch (lcp_p->remote.want.authentication) {
            case PPP_PAP_PROTOCOL:
