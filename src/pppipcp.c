...
static int16 ipcp_negotiate = IPCP_N_ADDRESSES|IPCP_N_COMPRESS|IPCP_N_ADDRESS;
...
static void ipcp_option __ARGS((struct mbuf **bpp, struct ipcp_value_s *value_p,
    byte_t o_type,byte_t o_length,struct mbuf **copy_bpp));
static void ipcp_makeoptions __ARGS((struct mbuf **bpp,struct ipcp_value_s *value_p,
    int16 negotiating));
static struct mbuf *ipcp_makereq __ARGS((struct fsm_s *fsm_p));
  
static int ipcp_check __ARGS((struct mbuf **bpp,struct ipcp_s *ipcp_p,
    struct ipcp_side_s *side_p,struct option_hdr *option_p,int request));
  
static int ipcp_request __ARGS((struct fsm_s *fsm_p,struct config_hdr *config,
    struct mbuf *data));
static int ipcp_ack __ARGS((struct fsm_s *fsm_p,struct config_hdr *config,
    struct mbuf *data));
static int ipcp_nak __ARGS((struct fsm_s *fsm_p,struct config_hdr *config,
    struct mbuf *data));
static int ipcp_reject  __ARGS((struct fsm_s *fsm_p,struct config_hdr *config,
    struct mbuf *data));
...
static struct cmds DFAR IPcpcmds[] = {
...
static struct cmds DFAR IPcpside_cmds[] = {
...
void *p;
{
    ...
    if (argc < 2) {
        ...
        return bit16cmd( &(side_p->will_negotiate), IPCP_N_ADDRESSES|IPCP_N_ADDRESS,
        "Allow Address", --argc, &argv[1] );
    }
    if ((x32 = (int32)stricmp(argv[1],"none")) != 0L && (x32 = resolve(argv[1])) == 0L) {
        tprintf(Badhost,argv[1]);
    }
    side_p->want.address = x32;
    side_p->want.negotiate |= (IPCP_N_ADDRESSES|IPCP_N_ADDRESS);
    return 0;
}
    ...
    || stricmp(argv[1],"vj") == 0 ) {
        ...
        if ( argc >= 3 ) {
            ...
            if ( side_p->want.slots < 1 || side_p->want.slots > 255 ) {
                j2tputs( "slots must be in range 1 to 255" );
                return 1;
            }
        } else {
...
struct mbuf **copy_bpp;
{
    ...
    *cp++ = o_length;
    ...
    switch ( o_type ) {
        case IPCP_ADDRESS:      /* LAK for rfc 1332 */
            both_addrs = 0;
            /* Fall into IPCP_ADDRESSES case */
...
int request;
{
    ...
    switch(option_p->type) {
        case IPCP_ADDRESS:    /* LAK for rfc1332 */
            both_addrs = 0;
            /* fall into IPCP_ADDRESSES case */
        ...
        case IPCP_ADDRESSES:
            ...
            /* override any undesirable changes */
                if (both_addrs && ipcp_p->remote.want.address != 0L) {
                    ipcp_p->local.work.other
                    = ipcp_p->remote.want.address;
                }
#ifdef PPP_NO_LOCALADDR_CHANGE
/* Refuse new local IP address suggestion when we already have one defined.
 * This breaks dynamic IP address assignment when we don't reattach before
 * redialing. -- n5knx
 */
            ...
            if (both_addrs) {
                if (ipcp_p->remote.work.address == ipcp_p->remote.want.address) {
                    ...
                    /* don't know address either */
						...
						ipcp_p->local.work.other = ipcp_p->remote.work.address; /* n5knx */
...
int32 addr;
{
    ...
    /* n5knx: now also looks at *remote* addresses */
    for (ifp=Ifaces; ifp != NULLIF; ifp = ifp->next) {
        if (ifp->addr == addr)
            return 0L;
        if (ifp->type == CL_PPP && ifp->edv && (fsm_p = &(((struct ppp_s *)ifp->edv)->fsm[IPcp])) != NULL
            && ((struct ipcp_s *)fsm_p->pdv)->local.work.other == addr)
            return 0L;
        /* we could also check out remote slip addrs?? */
    }
    return addr;
}
