...
/* log (-1, "argc [%d] sid [%s] [%s]", argc, argv[1], argv[2]);
	13Apr2010, maiko, debugging */
...
char *string;
{
    ...
    if((cp = strchr(string,'_')) != NULLCHAR && *(cp+1) != '\0' &&
        strnicmp(cp+1,(Mbhaddress?Mbhaddress:Hostname),strlen(cp+1)) == 0
        && *((Mbhaddress?Mbhaddress:Hostname)+strlen(cp+1)) == '.'
        && strspn(string,"0123456789") == (size_t)(cp-string) )
        return 1;
