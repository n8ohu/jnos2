...
int msgn;
{
    ...
    if(Mbheader) {
        ...
        usprintf(m->user," #:%ld",(f->ind.msgid % 100000L));  /* n5knx: use rightmost 5 digits */
        /* The BID, is any */
        if(f->bid[0] != '\0')
            ...
            while (
                ...
                if(
                   ...
                   ISPROMPT(m->line))
                    ...
                    logmbox (m->user, m->name, "sent %s", line);
                    if(m->areatype == AREA)
...
void *p;
{
    ...
    logmbox (f.m->user, f.m->name, "forwarding mail");
...
void *v1, *v2;
{
	...
	int telpac = 0;	/* 11Mar2006, Maiko, WL2K telpac uses only CR */
#endif
#ifdef	DONT_COMPILE
#ifdef	HFDD
	int hfdd = 0;
	...
	else	
	{
		...
		/* 25Sep2006, Maiko, Arggg, scratch the ! character - bad */
		if (strstr (cc, "telpac"))
			telpac = 1;
#endif
...
/* log (-1, "waiting for SID from remote bbs"); */
...
/*
 * 04May2010, Maiko, Crap, after all this time, it would seem that
 * this one line below which disables FBBCMP, screws up which of the
 * B2Fsendmsg or fbbsendmsg gets called. Discovered (FINALLY) upon
 * doing development work with the WINMOR tnc. Fought for several
 * days trying to figure out why certain code was never getting
 * run. It's because the MBX_FBBCMP flag was being cleared.
 */

#ifdef	HFDD
	/* 07Jan2005, Disable compression if PTC (ptc does it better by itself) */
#ifdef FBBCMP
	if (hfdd)
		m->sid &= ~MBX_FBBCMP;
#endif
...
#ifdef MB_XFBB_KLUDGE
	/* 11Mar2006, Maiko, For WinLink telpac connects */
	if (telpac)
		m->sid &= ~MBX_FBBCMP;
#endif
...
void *p;
{
    ...
    switch(*argv[0]) {
        ...
        /* get the optional port number */
            ...
            if((m->user = j2socket(AF_INET,SOCK_STREAM,0)) == -1) {
                if(Mtrace)
            ...
            if (argc > 3 &&
				(!stricmp(argv[3],"cronly") || !stricmp(argv[3], "telpac")))
			{
                struct usock *up;
